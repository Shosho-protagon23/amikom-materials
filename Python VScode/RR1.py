...
class RussianRoulette:
    def __init__(self, root):
        ...
        # Tombol baru untuk putar silinder
        self.spin_button = tk.Button(root, text="Putar Silinder", command=self.spin_cylinder)
    ...
    def spin_cylinder(self):
        # Putar silinder: acak ulang posisi peluru
        self.chambers = [False] * 6
        self.bullet_position = random.randint(0, 5)
        self.chambers[self.bullet_position] = True
        self.shots_taken = 0  # Reset shots
        self.current_player = "Player"  # Reset ke Player
        self.status_label.config(text=f"Giliran: {self.current_player}")
        self.label.config(text="Russian Roulette\nSilinder diputar ulang!\nPeluru di salah satu dari 6 chamber.")
        messagebox.showinfo("Silinder Diputar", "Posisi peluru diacak ulang. Mulai dari awal!")
